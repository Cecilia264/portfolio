# portfolio
# Hola, soy Cecilia Cometto

## Sobre mí
En el año 2025 finalice la ultima materia pendiente para ser Analista de Sistemas

## Tecnologías
- Backend: Python, FastAPI, SQLAlchemy, APScheduler
- Frontend: React, CSS3, HTML5, JavaScript 
- Base de datos: PostgreSQL
- Visualización: Matplotlib/Plotly
- Herramientas: Git

## 📂 Proyectos Destacados

## **→ Sistema de Gestión de Turnos Médicos**  
Aplicación web completa para digitalizar la administración de consultorios y clínicas, 
con gestión de pacientes, médicos, turnos, historial clínico y reportes automatizados.

- Características principales:
  - API REST con FastAPI y arquitectura por capas
  - Sistema de persistencia con PostgreSQL y SQLAlchemy ORM
  - Generación de reportes y visualizaciones con Matplotlib/Plotly
  - Programación de tareas automáticas (recordatorios) con APScheduler
  - Interfaz web responsiva y accesible

- 🛠️ Tecnologías usadas:
  - Backend: Python, FastAPI, SQLAlchemy, APScheduler
  - Base de datos: PostgreSQL
  - Visualización: Matplotlib / Plotly
  - Frontend: React, CSS3, HTML5, JavaScript
  - Herramientas: Git

- Enlaces:
🔗 Repositorio en GitHub
🚀 Demo en vivo (si tienes deployment)
📋 Documentación de la API (si la documentaste con Swagger/Redoc)

- 📦 Instalación
  Clonar repositorio
  git clone https://github.com/tuusuario/nombre-proyecto.git

  Instalar dependencias
  pip install -r requirements.txt

  Configurar variables de entorno
  cp .env.example .env

  Ejecutar aplicación
  uvicorn main:app --reload

  Ejecutar frontend
  npm start


**Sistema de Gestión de CLub Deportivo**
Sistema desarrollado en Python para la administración integral de un club deportivo. 
Permite gestionar socios, calcular recaudaciones mensuales y generar reportes estadísticos 
según tres categorías de membresía con reglas de cobro diferenciadas.

- Características principales:
  - Programación Orientada a Objetos con herencia y polimorfismo
  - Procesamiento de archivos CSV con datos de socios
  - Cálculo inteligente de cuotas según tipo de socio
  - Detección estacional automática (meses de verano desde sistema)
  - Generación de múltiples reportes y estadísticas
  - Sistema modular con clases bien definidas

- 🛠️ Tecnologías usadas:
  - Lenguaje: Python 3.13.2
  - Paradigma: Programación Orientada a Objetos (POO)
  - Patrones: Herencia, Polimorfismo, Clases Abstractas
  - Manejo de archivos: CSV processing
  - Testing: pytest (test_socios.py)
  - Control de versiones: Git (.gitignore, .github)

- Enlaces:
🔗 Repositorio en GitHub
🚀 Demo en vivo (si tienes deployment)
📋 Documentación de la API (si la documentaste con Swagger/Redoc)

- 📦 Instalación
  Clonar repositorio
  git clone https://github.com/tuusuario/nombre-proyecto.git

  Ejecutar aplicación
  uvicorn main:app --reload



